...
void goToCoordinates (Vector2 pos)
{
	HANDLE outputHandle = GetStdHandle(STD_OUTPUT_HANDLE);
	COORD coord;
...
void drawFrame (Vector2 upLeft, Vector2 lowRight) 
{
	const float x1 = upLeft.x;
	const float x2 = lowRight.x;
	const float y1 = upLeft.y;
	const float y2 = lowRight.y;

	goToCoordinates({x1, y1});
	std::cout << (char)LEFT_UP;
	goToCoordinates({x2, y1});
	std::cout << (char)RIGHT_UP;
	goToCoordinates({x2, y2});
	std::cout << (char)RIGHT_DW;
	goToCoordinates({x1, y2});
	std::cout << (char)LEFT_DW;
	
	for (float i = x1 + 1; i < x2; i++) 
	{
		goToCoordinates({i, y1});
		std::cout << (char)LINE_HOR;
		goToCoordinates({i, y2});
		std::cout << (char)LINE_HOR;
	}
	
	for (float i = y1 + 1; i < y2; i++) 
	{
		goToCoordinates({x2, i});
		std::cout << (char)LINE_VER;
